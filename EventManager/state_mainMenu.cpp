...
void State_MainMenu::MouseClick(EventDetails *details) {
	...
	for (int i = 0; i < 3; ++i) {
		//origin of button = absolute middle
		float rectPosX = m_rects[i].getPosition().x;
		float rectPosY = m_rects[i].getPosition().y;
		if (mousePos.x >= rectPosX - halfX && mousePos.x <= rectPosX + halfX &&
			mousePos.y >= rectPosY - halfY && mousePos.y <= rectPosY + halfY) {
			if (i == 0) {
